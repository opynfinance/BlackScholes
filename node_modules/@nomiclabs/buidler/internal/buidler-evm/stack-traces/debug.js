...
function printCreateTrace(trace, depth) {
    ...
    if (trace.bytecode !== undefined) {
        console.log(`${margin} deploying contract: ${trace.bytecode.contract.location.file.globalName}:${trace.bytecode.contract.name}`);
...
function printCallTrace(trace, depth) {
    ...
    if (trace.bytecode !== undefined) {
        console.log(`${margin} calling contract: ${trace.bytecode.contract.location.file.globalName}:${trace.bytecode.contract.name}`);
    }
    else {
        ...
        if (message_trace_1.isEvmStep(step)) {
            ...
            if (trace.bytecode !== undefined) {
                ...
                if (inst.location !== undefined) {
                    ...
                    if (func !== undefined) {
                        location += `:${func.contract.name}:${func.name}`;
                    }
...
function printStackTrace(trace) {
    const withDecodedMessages = trace.map((entry) => entry.type === solidity_stack_trace_1.StackTraceEntryType.REVERT_ERROR
        ? Object.assign(Object.assign({}, entry), { message: revert_reasons_1.decodeRevertReason(entry.message) }) : entry);
