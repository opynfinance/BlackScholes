...
class EthModule {
    constructor(_common, _node, _throwOnTransactionFailures, _throwOnCallFailures, _logger) {
    ...
    async _callAction(rpcCall, blockTag) {
        ...
        const { result: returnData, trace, error, consoleLogMessages, } = await this._node.runCall(callParams, this._shouldCallOnNewBlock(blockTag));
    ...
    async _estimateGasAction(transactionRequest, blockTag) {
        ...
        const { estimation, error, trace, consoleLogMessages, } = await this._node.estimateGas(txParams);
        if (error !== undefined) {
    ...
    async _validateBlockTag(blockTag) {
        const latestBlock = await this._node.getLatestBlockNumber();
        if (ethereumjs_util_1.BN.isBN(blockTag) && latestBlock.eq(blockTag)) {
            return;
        }
        ...
        if (blockTag !== undefined &&
            blockTag !== "latest" &&
            blockTag !== "pending") {
            throw new errors_1.InvalidInputError(`Received block param ${blockTag.toString()} and latest block is ${latestBlock.toString()}.
...
If this error persists, try resetting your wallet's accounts.`);
    ...
    _shouldCallOnNewBlock(blockTag) {
