    ...
    abi.forEach(abiPiece => {
        ...
        if (abiPiece.type === "event") {
            ...
            if (eventAbi.anonymous) {
                debug_1.default(`Skipping anonymous event... ${JSON.stringify(eventAbi)}`);
                return;
            }
...
function parseFallback(abiPiece) {
    debug_1.default(`Parsing fallback declaration`);
    return {
        name: "fallback",
        inputs: [],
...
function parseRawAbiParameterType(rawAbiParameter) {
    ...
    return parseEvmType_1.parseEvmType(rawAbiParameter.type, components);
}
